...
interface Props {
  ...
  position?: "miniAppBar" | "AppBar";
}

const Searchbar = React.forwardRef(({ toggleSearch, position }: Props, ref) => {
  ...
  return (
    <Box ref={ref}>
      <Container maxWidth="lg" fixed>
        <Box sx={{ position: "relative" }} paddingLeft={position === "miniAppBar" ? 0 : smScreen ? "100px" : "130px"} alignSelf={["flex-end"]}>
          <Input
            type="text"
            fullWidth
            placeholder="Search..."
          ...
          <Box
            sx={{
              position: "absolute",
              width: `${position === "miniAppBar" ? "100%" : `calc(100% - ${smScreen ? "100px" : "130px"})`}`,
              background: "#f7f7f7",
              zIndex: 1200,
